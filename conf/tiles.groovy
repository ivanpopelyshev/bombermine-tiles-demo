void defaultTile(Map tile) { tileConf.defaultTile(tile); }
Map newGroup(String name, Map attr) { return tileConf.newGroup(name, attr); }
Map newTile(String name, Map attr) { return tileConf.newTile(name, attr); }
Map newSlot(String name, Map attr) { return tileConf.newSlot(name, attr); }
Map newItem(String name, Map attr) { return tileConf.newItem(name, attr); }
Map newChange(String name, Map attr) { return tileConf.newChange(name, attr); }
Map sub(String name, Map attr) {
    return tileConf.newTile(name, attr);
}
Map sub(String name) {
    return tileConf.newTile(name, new HashMap());
}
Map getTile(String name) {return tileConf.getTile(name); }
Map getSlot(String name) {return tileConf.getSlot(name); }
Map getItem(String name) { return tileConf.getItem(name); }
Map getChange(String name) { return tileConf.getChange(name); }
Map getItem(Stringname) { return tileConf.getItem(name); }

def BLOCK_EFFECT_SPAWN_BOMBS = 1;
def BLOCK_EFFECT_FORCE_REGEN = 2;
def BLOCK_EFFECT_DESTROY_BLOCK = 3;
def BLOCK_EFFECT_SPAWN_BOMBS_NY = 4;

def ITEM_EFFECT_CHANGE_COUNT_TO_BOMBS_PLUS_TWO = 5;
def ITEM_EFFECT_TELEPORT_FROM_STRUCTURE = 6;
def ITEM_EFFECT_RANDOM_DESEASE = 7;
def ITEM_EFFECT_XOR = 8;
def ITEM_EFFECT_BALL = 9;
def ITEM_EFFECT_SANTA_CANT_TAKE = 10;

def next = getChange("next");
    ...
    subTiles: [
        sub("rock3", [
            onDamage: newChange("next_or_chest", [
                variants: [[p: 9,result: next    ], [p: 1, result: getTile("chest_in_rock") ]]
            ])
        ]),
        sub("rock2"),
        sub("rock1"),
